...
type MockAddCragHandler struct {
	Handler func(ctx context.Context, command *dto.AddCragRequest) error
}

func (m MockAddCragHandler) Handle(ctx context.Context, command *dto.AddCragRequest) error {
...
func TestCragHandler_AddCrag(t *testing.T) {
	var tests = []struct {
	...
	}{
		{
			...
			handler: MockAddCragHandler{Handler: func(ctx context.Context, command *dto.AddCragRequest) error {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := NewCragHttpApi(app.Application{Commands: app.Commands{AddCragHandler: tt.handler}})
